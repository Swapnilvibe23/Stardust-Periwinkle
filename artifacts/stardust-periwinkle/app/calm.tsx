...
export default function CalmScreen() {
  ...
  async function generate() {
    ...
    if (newCount >= DAILY_LIMITS.calm) setShowUpgradeBanner(true);
  }
  ...
  return (
    ...
    >
      ...
      {/* Saved calm moments shortcut */}
      <FavoritesBoost type="calm" limit={2} />
      ...
      <View style={s.chipRow}>
        {CALM_SCENARIOS.map((sc) => (
          <TouchableOpacity
            ...
            onPress={() => { setScenario(sc); setResult(null); }}
          >
      ...
      {loaded && (
        <UsageIndicator remaining={remaining} limit={DAILY_LIMITS.calm} featureName="Calm This Moment" />
      )}
      ...
      {result && (
        <View style={s.resultContainer}>
          <ResultStep step="1" label="What to Say" content={result.whatToSay} color="#B83A6B" colors={colors} />
          <ResultStep step="2" label="What to Do" content={result.whatToDo} color="#7B8ECC" colors={colors} />
          <ResultStep step="3" label="What to Try" content={result.whatToTry} color="#6B5B95" colors={colors} />
...
function ResultStep({
  step, label, content, color, colors,
}: {
  step: string; label: string; content: string; color: string; colors: ReturnType<typeof useColors>;
}) {
  return (
...
const rstyles = StyleSheet.create({
  card: {
    flexDirection: "row", borderRadius: 14, padding: 14, marginBottom: 10, gap: 12,
    shadowColor: "#000", shadowOpacity: 0.04, shadowRadius: 4,
    shadowOffset: { width: 0, height: 1 }, elevation: 1,
  },
  badge: { width: 28, height: 28, borderRadius: 14, alignItems: "center", justifyContent: "center", marginTop: 2, flexShrink: 0 },
...
function makeStyles(colors: ReturnType<typeof useColors>) {
  return StyleSheet.create({
    ...
    warningBanner: {
      ...
      backgroundColor: "#FFF0F4", borderRadius: 10, padding: 12, marginBottom: 16,
    },
    ...
    timeChip: {
      flex: 1, paddingVertical: 10, borderRadius: 12, backgroundColor: colors.muted,
    ...
    ageNote: { flexDirection: "row", alignItems: "center", gap: 5, marginTop: 12, marginBottom: 2 },
    ageNoteText: { fontSize: 12, color: colors.primary, fontWeight: "500" as const },
    generateButton: { backgroundColor: colors.primary, borderRadius: 14, padding: 16, alignItems: "center", marginTop: 12 },
    generateButtonAtLimit: { backgroundColor: "#C98DAA" },
    generateButtonText: { color: "#fff", fontSize: 16, fontWeight: "700" as const },
    resultContainer: { marginTop: 20 },
    extraCard: { backgroundColor: "#FFF5E6", borderRadius: 14, padding: 14, marginBottom: 10 },
    ...
    saveButton: {
      ...
      gap: 8, borderWidth: 2, borderColor: "#B83A6B", borderRadius: 12, padding: 14, marginTop: 8,
    },
